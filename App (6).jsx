...
// ─── Discount Code Master Registry ───────────────────────────────────────────
// Each code has: id, label, category (service_recovery|marketing|staff|promotional),
// useCase, plCategory, hasCOGS (bool), hasShipping (bool)
const DISCOUNT_CODE_REGISTRY = [
  // Service Recovery
  { id:"RESHIP-FAULTY",  label:"RESHIP-FAULTY",  category:"service_recovery", useCase:"Replacement for faulty/wrong garment — product at $0, customer pays shipping or free", plCategory:"Warranty / COGS Expense", hasCOGS:true, hasShipping:true },
  { id:"RESHIP-LOST",    label:"RESHIP-LOST",    category:"service_recovery", useCase:"Replacement for lost parcel (AusPost claim)", plCategory:"Logistics Loss / COGS", hasCOGS:true, hasShipping:true },
  { id:"RESHIP-DAMAGED", label:"RESHIP-DAMAGED", category:"service_recovery", useCase:"Garment damaged in transit, replacement sent", plCategory:"Logistics Loss / COGS", hasCOGS:true, hasShipping:true },
  { id:"RESHIP-RTS",     label:"RESHIP-RTS",     category:"service_recovery", useCase:"Return-to-sender reshipment — customer paid, you're resending", plCategory:"Logistics / Operational", hasCOGS:false, hasShipping:true },
  { id:"RESHIP-CUSTOMS", label:"RESHIP-CUSTOMS", category:"service_recovery", useCase:"Parcel failed customs, being resent free of charge", plCategory:"Logistics Loss", hasCOGS:true, hasShipping:true },
  { id:"EXCHANGE-SE",    label:"EXCHANGE-SE",    category:"service_recovery", useCase:"Size exchange — customer paid, you're sending correct size", plCategory:"Customer Retention Cost / COGS", hasCOGS:true, hasShipping:true },
  { id:"EXCHANGE-GIFT",  label:"EXCHANGE-GIFT",  category:"service_recovery", useCase:"Exchange absorbed as goodwill gesture", plCategory:"CS Goodwill Expense", hasCOGS:true, hasShipping:true },
  { id:"CS-ERROR",       label:"CS-ERROR",       category:"service_recovery", useCase:"We packed wrong item — replacement sent at our cost", plCategory:"Operational Error / COGS", hasCOGS:true, hasShipping:true },
  // Staff
  { id:"FM2USTAFF",      label:"FM2USTAFF",      category:"staff",            useCase:"Staff purchases", plCategory:"Staff Benefit / COGS", hasCOGS:false, hasShipping:false },
  // Marketing
  { id:"COLLAB2026",     label:"COLLAB2026",     category:"marketing",        useCase:"Marketing / Influencer collaboration", plCategory:"Customer Acquisition Cost (Marketing)", hasCOGS:false, hasShipping:false },
  // Promotional — users add their own
];

// Category config
const DISC_CATEGORIES = [
  { id:"service_recovery", label:"Service Recovery", sub:"Reshipments, exchanges, warranty, CS errors — reclassified as operational/COGS expense.", badge:"Reclassified as: COGS / Operational Expense", color:"RD" },
  { id:"marketing",        label:"Marketing / Influencer", sub:"Collab codes, gifting — reclassified as marketing expense.", badge:"Reclassified as: Marketing Expense (OPEX)", color:"YL" },
  { id:"staff",            label:"Staff Discounts", sub:"Staff purchase discounts — reclassified as staff benefits expense.", badge:"Reclassified as: Staff Benefits (Wages)", color:"A" },
  { id:"promotional",      label:"Promotional / Sale", sub:"True sale codes — the only bucket that stays as a revenue deduction.", badge:"Stays as: Revenue Deduction", color:"GR" },
];

// Legacy bucket structure kept for backwards compat during calc
...
// Per-code data store: { [codeId]: { orders, retailValue, cogsValue, shippingValue, active } }
function emptyCodeData(){
  const out={};
  DISCOUNT_CODE_REGISTRY.forEach(c=>{
    out[c.id]={orders:"",retailValue:"",cogsValue:"",shippingValue:"",active:true};
  });
  return out;
}

// Derive legacy discBuckets from per-code data for backwards-compat calcs
function codeDataToDiscBuckets(codeData){
  const totals={service_recovery:{retailValue:0,orders:0,cogsValue:0},marketing:{retailValue:0,orders:0,cogsValue:0},staff:{retailValue:0,orders:0,cogsValue:0},promotional:{retailValue:0,orders:0,cogsValue:0}};
  DISCOUNT_CODE_REGISTRY.forEach(c=>{
    const d=codeData?.[c.id];
    if(!d||!d.active)return;
    const cat=c.category;
    if(totals[cat]){
      totals[cat].retailValue+=parseFloat(d.retailValue)||0;
      totals[cat].orders+=parseFloat(d.orders)||0;
      totals[cat].cogsValue+=(parseFloat(d.cogsValue)||0)+(parseFloat(d.shippingValue)||0);
    }
  });
  const out={};
  DEFAULT_DISC_BUCKETS.forEach(b=>{
    const t=totals[b.id]||{};
    out[b.id]={retailValue:t.retailValue||"",orders:t.orders||"",cogsValue:t.cogsValue||"",codes:b.defaultCodes};
  });
  return out;
}
...
function calcWeek(week,fixed,opexKeys,depts){
  ...
  // Use codeData if available (new per-code system), else fall back to discBuckets
  const effectiveBuckets = week.codeData
    ? codeDataToDiscBuckets(week.codeData)
    : (week.discBuckets || emptyDiscBuckets());
...
function generateExport(weeks,fixed,extras,mLabel,opexKeys,depts,staff,labels){
  ...
  // Per-code breakdown
  o+="--- PER-CODE BREAKDOWN ---\n";
  DISCOUNT_CODE_REGISTRY.forEach(code=>{
    const weekTotals=weeks.reduce((acc,w)=>{
      const d=w.codeData?.[code.id]||{};
      return { orders:acc.orders+n(d.orders), retail:acc.retail+n(d.retailValue), cogs:acc.cogs+n(d.cogsValue), shipping:acc.shipping+n(d.shippingValue) };
    },{orders:0,retail:0,cogs:0,shipping:0});
    if(weekTotals.orders===0&&weekTotals.retail===0)return;
    const totalCost=weekTotals.cogs+weekTotals.shipping;
    const costPerOrder=weekTotals.orders>0?totalCost/weekTotals.orders:0;
    o+="  "+code.label+" ["+code.plCategory+"]: "+weekTotals.orders+" orders | Retail: "+fmt(weekTotals.retail)+" | COGS: "+fmt(weekTotals.cogs)+" | Shipping: "+fmt(weekTotals.shipping)+(totalCost>0?" | Total P&L hit: "+fmt(totalCost):"")+(costPerOrder>0?" | Avg: "+fmt(costPerOrder)+"/order":"")+"\n";
  });
  o+="\n";
...
// ─── Discount Breakdown Section (per-code) ────────────────────────────────────
function DiscountBreakdown({week,onChange,labels}){
  ...
  // codeData: { [codeId]: { orders, retailValue, cogsValue, shippingValue, active } }
  const codeData=week.codeData||(()=>{
    // migrate from old discBuckets if needed
    return emptyCodeData();
  })();

  const upCode=(id,field,val)=>{
    const nd={...codeData,[id]:{...codeData[id],[field]:val}};
    onChange({...week,codeData:nd});
  };
  const toggleCode=(id)=>{
    upCode(id,"active",!codeData[id]?.active);
  };
  ...
  DISCOUNT_CODE_REGISTRY.forEach(code=>{
    const d=codeData[code.id]||{};
    if(!d.active&&d.active!==undefined&&d.active!==null?false:true){
      const cat=catTotals[code.category];
      if(cat){
        cat.retail+=n(d.retailValue);
        cat.orders+=n(d.orders);
        cat.cogs+=n(d.cogsValue);
        cat.shipping+=n(d.shippingValue);
      }
    }
  });
  ...
  // Grand totals
  const totalRetailAllocated=Object.values(catTotals).reduce((s,c)=>s+c.retail,0);
  const unallocated=totalDiscounts-totalRetailAllocated;

  // Promotional discount rate analysis
  const promoRetail=catTotals.promotional?.retail||0;
  const grossSales=n(week.revenue.gross_sales);
  const promoRate=grossSales>0?(promoRetail/grossSales)*100:0;
  const promoAlert=promoRate>15;

  // Service recovery cost per order
  const srOrders=catTotals.service_recovery?.orders||0;
  const srTotalCost=(catTotals.service_recovery?.cogs||0)+(catTotals.service_recovery?.shipping||0);
  const srCostPerOrder=srOrders>0?srTotalCost/srOrders:0;

  return(
    <Accordion title={<E value={labels.disc_section||"Discount Code Breakdown"} onSave={v=>labels._save("disc_section",v)} style={{fontFamily:ff,fontSize:10,color:A}}/>} accent>
      <div style={{fontFamily:ff,fontSize:12,color:MU,marginBottom:16,lineHeight:1.7}}>
        {labels.disc_section_sub||"Reclassifies each discount code into its true cost category. Enter data per code — totals auto-aggregate into P&L buckets."}
      </div>

      {/* Summary allocation bar */}
      <div style={{background:S2,borderRadius:radius+1,padding:"12px 14px",marginBottom:20}}>
          ...
          {DISC_CATEGORIES.map(cat=>{
            ...
            return(
              <div key={cat.id} style={{flex:1,minWidth:90,background:BG,borderRadius:3,padding:"7px 10px",borderLeft:"3px solid "+col}}>
                <div style={{fontFamily:ff,fontSize:9,color:MU,textTransform:"uppercase",letterSpacing:0.8,marginBottom:3}}>{cat.label}</div>
                <div style={{fontFamily:ff,fontSize:13,color:col,fontWeight:"bold"}}>{fmtD(t.retail||0)}</div>
                <div style={{fontFamily:ff,fontSize:10,color:MU,marginTop:1}}>{t.orders||0} orders</div>
              </div>
            );
          })}
        </div>
        {Math.abs(unallocated)>0.01&&(
          <div style={{fontFamily:ff,fontSize:11,color:Math.abs(unallocated)>1?RD:MU}}>
            {Math.abs(unallocated)>1?"⚠ Unallocated: "+fmtD(unallocated)+" — enter code data below":"✓ Fully allocated"}
          </div>
        )}
      </div>
      ...
      {DISC_CATEGORIES.map(cat=>{
        const col=catColors[cat.id]||A;
        const codes=DISCOUNT_CODE_REGISTRY.filter(c=>c.category===cat.id);
        const t=catTotals[cat.id]||{};
        const totalCost=(t.cogs||0)+(t.shipping||0);
        return(
          <div key={cat.id} style={{marginBottom:20}}>
            {/* Category header */}
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",borderBottom:"1px solid "+col+"55",paddingBottom:8,marginBottom:12}}>
              <div>
                <span style={{fontFamily:ff,fontSize:11,color:col,letterSpacing:1.5,textTransform:"uppercase",fontWeight:"bold"}}>{cat.label}</span>
                <span style={{fontFamily:ff,fontSize:10,color:MU,marginLeft:10}}>{cat.sub}</span>
              </div>
              <div style={{display:"flex",gap:8,alignItems:"center"}}>
                {totalCost>0&&(
                  <span style={{fontFamily:ff,fontSize:10,color:MU}}>Total cost: <span style={{color:RD}}>{fmtD(totalCost)}</span></span>
                )}
                <div style={{background:col+"22",border:"1px solid "+col+"44",borderRadius:3,padding:"3px 8px",fontFamily:ff,fontSize:9,color:col,letterSpacing:0.8,textTransform:"uppercase",whiteSpace:"nowrap"}}>
                  {cat.badge}
                </div>
              </div>
            </div>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:10}}>
              {codes.map(code=>{
                ...
                const loss=codeCost>0?codeCost:codeRetail; // cost of this issue
                const orders=n(d.orders);
                const costPerOrder=orders>0&&loss>0?loss/orders:0;

                return(
                  <div key={code.id} style={{background:S2,border:"1px solid "+(hasData?col+"44":BR),borderRadius:radius+1,padding:"12px 14px",opacity:active?1:0.45,transition:"opacity 0.2s"}}>
                    {/* Code header row */}
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:hasData?10:0}}>
                      <div style={{flex:1}}>
                        <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:3}}>
                          <span style={{fontFamily:"monospace",fontSize:12,color:col,fontWeight:"bold",letterSpacing:0.5}}>{code.label}</span>
                          {hasData&&costPerOrder>0&&(
                            <span style={{fontFamily:ff,fontSize:10,color:MU,background:BG,border:"1px solid "+BR,borderRadius:2,padding:"1px 6px"}}>
                              {fmtD(costPerOrder)} / order
                            </span>
                          )}
                          {hasData&&loss>0&&(
                            <span style={{fontFamily:ff,fontSize:10,color:RD,background:RD+"11",border:"1px solid "+RD+"33",borderRadius:2,padding:"1px 6px"}}>
                              −{fmtD(loss)} total
                            </span>
                          )}
                        </div>
                        <div style={{fontFamily:ff,fontSize:10,color:MU,lineHeight:1.5}}>{code.useCase}</div>
                        <div style={{fontFamily:ff,fontSize:9,color:col+"aa",marginTop:2,textTransform:"uppercase",letterSpacing:0.6}}>{code.plCategory}</div>
                      </div>
                      <button onClick={()=>toggleCode(code.id)}
                        style={{background:"transparent",border:"1px solid "+BR,borderRadius:2,padding:"3px 8px",fontFamily:ff,fontSize:9,color:active?MU:col,cursor:"pointer",letterSpacing:0.5,textTransform:"uppercase",marginLeft:10,whiteSpace:"nowrap"}}>
                        {active?"Hide":"Show"}
                      </button>
                    </div>
                    ...
                    {active&&(
                      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(130px,1fr))",gap:8,marginTop:8}}>
                        <Fld label={<span style={{fontFamily:ff,fontSize:10,color:MU,textTransform:"uppercase",letterSpacing:0.7}}>Orders</span>}>
                          <NI value={d.orders} onChange={v=>upCode(code.id,"orders",v)}/>
                        </Fld>
                        <Fld label={<span style={{fontFamily:ff,fontSize:10,color:MU,textTransform:"uppercase",letterSpacing:0.7}}>Retail Value Discounted</span>}>
                          <CI value={d.retailValue} onChange={v=>upCode(code.id,"retailValue",v)}/>
                        </Fld>
                        {code.hasCOGS&&(
                          <Fld label={<span style={{fontFamily:ff,fontSize:10,color:MU,textTransform:"uppercase",letterSpacing:0.7}}>Mfg COGS of goods sent</span>}>
                            <CI value={d.cogsValue} onChange={v=>upCode(code.id,"cogsValue",v)}/>
                          </Fld>
                        )}
                        {code.hasShipping&&(
                          <Fld label={<span style={{fontFamily:ff,fontSize:10,color:MU,textTransform:"uppercase",letterSpacing:0.7}}>Shipping Cost</span>}>
                            <CI value={d.shippingValue} onChange={v=>upCode(code.id,"shippingValue",v)}/>
                          </Fld>
                        )}
                      </div>
                    )}
            ...
            {(t.retail>0||t.cogs>0||t.shipping>0)&&(
              <div style={{display:"flex",gap:8,flexWrap:"wrap",marginTop:10,padding:"10px 12px",background:col+"0d",border:"1px solid "+col+"22",borderRadius:radius}}>
                <span style={{fontFamily:ff,fontSize:10,color:MU,flex:1,textTransform:"uppercase",letterSpacing:0.8}}>{cat.label} subtotal</span>
                <span style={{fontFamily:ff,fontSize:10,color:MU}}>Orders: <b style={{color:TX}}>{t.orders||0}</b></span>
                <span style={{fontFamily:ff,fontSize:10,color:MU}}>Retail discounted: <b style={{color:col}}>{fmtD(t.retail||0)}</b></span>
                {(t.cogs>0||t.shipping>0)&&<span style={{fontFamily:ff,fontSize:10,color:MU}}>COGS: <b style={{color:RD}}>{fmtD(t.cogs||0)}</b></span>}
                {t.shipping>0&&<span style={{fontFamily:ff,fontSize:10,color:MU}}>Shipping: <b style={{color:RD}}>{fmtD(t.shipping||0)}</b></span>}
                {(t.cogs>0||t.shipping>0)&&<span style={{fontFamily:ff,fontSize:10,color:RD,fontWeight:"bold"}}>Total P&L hit: {fmtD((t.cogs||0)+(t.shipping||0))}</span>}
              </div>
            )}
          </div>
        );
      ...
      {/* Analysis panel */}
      {(srOrders>0||promoRetail>0)&&(
        <div style={{marginTop:20,background:S2,border:"1px solid "+BR,borderRadius:radius+1,padding:"14px 16px"}}>
          <div style={{fontFamily:ff,fontSize:10,letterSpacing:2,color:A,textTransform:"uppercase",marginBottom:12}}>Issue Analysis</div>
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(220px,1fr))",gap:12}}>
            {srOrders>0&&(
              <div style={{background:BG,borderRadius:radius,padding:"10px 12px",borderLeft:"3px solid "+RD}}>
                <div style={{fontFamily:ff,fontSize:10,color:RD,textTransform:"uppercase",letterSpacing:0.8,marginBottom:6}}>Service Recovery</div>
                <div style={{fontFamily:ff,fontSize:11,color:TX,marginBottom:4}}>{srOrders} orders affected this week</div>
                <div style={{fontFamily:ff,fontSize:11,color:MU}}>Avg cost per issue: <span style={{color:RD,fontWeight:"bold"}}>{fmtD(srCostPerOrder)}</span></div>
                <div style={{fontFamily:ff,fontSize:11,color:MU}}>Annualised estimate: <span style={{color:RD,fontWeight:"bold"}}>{fmtD(srTotalCost*52)}</span></div>
                {srCostPerOrder>50&&<div style={{fontFamily:ff,fontSize:10,color:YL,marginTop:6}}>⚠ High cost per order — review root cause and address process failure</div>}
              </div>
            )}
            {promoRetail>0&&(
              <div style={{background:BG,borderRadius:radius,padding:"10px 12px",borderLeft:"3px solid "+(promoAlert?RD:GR)}}>
                <div style={{fontFamily:ff,fontSize:10,color:promoAlert?RD:GR,textTransform:"uppercase",letterSpacing:0.8,marginBottom:6}}>Promotional Discounts</div>
                <div style={{fontFamily:ff,fontSize:11,color:TX,marginBottom:4}}>{fmtD(promoRetail)} discounted this week</div>
                <div style={{fontFamily:ff,fontSize:11,color:MU}}>Promo rate: <span style={{color:promoAlert?RD:GR,fontWeight:"bold"}}>{promoRate.toFixed(1)}% of gross sales</span></div>
                {promoAlert&&<div style={{fontFamily:ff,fontSize:10,color:RD,marginTop:6}}>⚠ Exceeds 15% threshold — high discount rate is eroding margin. Consider reducing frequency or depth of sales.</div>}
                {!promoAlert&&<div style={{fontFamily:ff,fontSize:10,color:GR,marginTop:6}}>✓ Within healthy range (under 15%)</div>}
              </div>
            )}
          </div>
        </div>
