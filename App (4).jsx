...
const DEFAULT_THEME = {
  ...
  borderRadius: 4,
  // lightness modifiers 0-100
  lightness: 50,
};

function applyLightness(hex, lightness) {
  // lightness 0=original, 0-50 = darken, 50-100 = lighten
  const factor = (lightness - 50) / 50; // -1 to 1
  const r = parseInt(hex.slice(1,3),16);
  const g = parseInt(hex.slice(3,5),16);
  const b = parseInt(hex.slice(5,7),16);
  const adj = (c) => {
    if (factor > 0) return Math.min(255, Math.round(c + (255 - c) * factor * 0.7));
    return Math.max(0, Math.round(c + c * factor * 0.7));
  };
  return "#" + [adj(r),adj(g),adj(b)].map(v=>v.toString(16).padStart(2,"0")).join("");
}

function buildTheme(t) {
  const l = t.lightness ?? 50;
  const ap = hex => applyLightness(hex, l);
  return {
    A: ap(t.accent),
    BG: ap(t.bg),
    S: ap(t.surface),
    ...
    GR: ap(t.green),
    YL: ap(t.yellow),
    ff: t.bodyFont || "Times New Roman",
    ffTitle: t.titleFont || "Times New Roman",
...
const DEFAULT_LABELS = {
  // Nav
  tab_input: "WEEKLY INPUT",
  tab_overview: "MONTHLY OVERVIEW",
  tab_visualise: "VISUALISE",
  tab_compare: "COMPARE",
  tab_fixed: "FIXED COSTS",
  tab_reports: "REPORTS",
  tab_settings: "SETTINGS",
  // Header
  header_brand: "Finance Operations",
  header_title: "P&L Dashboard",
  header_subtitle: "weeks auto-dated Mon-Sun",
  // Sections - Weekly Input
  sec_shopify: "Shopify Data Import",
  sec_shopify_btn: "AUTOFILL FROM DATA",
  sec_revenue: "Revenue and Deductions",
  sec_cogs: "COGS - Cost of Goods",
  ...
  sec_freight: "Customer Shipping and Freight",
  sec_freight_sub: "Freight costs - included in total OPEX above.",
  sec_collabs: "Collaborations and Influencers",
  sec_collabs_sub: "Full cost breakdown per collaboration - all component costs included in total OPEX above.",
  sec_wages: "Staff Wages - By Department",
  sec_wages_sub: "Wages by department - included in total OPEX above.",
  sec_general: "General Operating Costs",
  sec_notes: "Notes / Context",
  sec_summary: "Weekly P&L Summary",
  // Field labels - Revenue
  field_gross_sales: "Gross Sales",
  field_refunds: "Refunds / Returns",
  field_discounts: "Discounts",
  field_shipping_income: "Shipping Income",
  field_paypal_fees: "PayPal Fees",
  field_net_revenue: "Net Revenue",
  // COGS
  field_mfg_product: "Manufacturing - Product COGS",
  field_mfg_shipping: "Manufacturing Shipping (Inbound Freight)",
  field_satchel_count: "Number of Orders (Satchels)",
  field_satchel_cost: "Cost Per Satchel ($)",
  field_satchel_total: "Satchel Total",
  field_other_pkg: "Other Packaging",
  field_total_cogs: "Total COGS",
  field_gross_profit: "Gross Profit",
  field_gross_margin: "Gross Margin",
  // Summary
  field_total_expenses: "Total Expenses",
  field_net_profit: "Net Profit",
  field_net_margin: "Net Margin",
  // Buttons
  btn_generate_export: "GENERATE EXPORT",
  btn_generate_export_sub: "paste into claude for deep insights",
  btn_monthly_summary: "Generate Monthly Summary",
  btn_monthly_summary_sub: "copy for notion / export",
  btn_compare_export: "GENERATE EXPORT",
  btn_compare_export_sub: "comparative analysis export for claude",
  btn_weekly_budget_export: "GENERATE BUDGET PLAN",
  btn_weekly_budget_export_sub: "next week staffing and budget guide",
  // Fixed costs
  fixed_help: "Enter recurring costs and mark which ones auto-populate weekly OPEX. Click SET FIXED to enable auto-population.",
  fixed_satchel_label: "Default Satchel Cost",
  fixed_satchel_sub: "per satchel (used when weekly input is blank)",
  // Monthly overview
  overview_part1: "PART 1 - SUMMARY",
  overview_part2: "PART 2 - MONTHLY ADJUSTMENTS",
  overview_adjustments_help: "Enter costs billed monthly not captured in weekly inputs. These are added to the monthly total.",
  // Compare
  compare_title: "Comparative Analysis",
  compare_help: "Select two periods to compare performance side by side.",
};
...
// Default staff roster for budget planning
const DEFAULT_STAFF = [
...
function emptyOpex(opexKeys){ const o={}; (opexKeys||DEFAULT_OPEX_KEYS).forEach(({key})=>{o[key]="";}); return o; }
function emptyWeek(weekNum,dateRange,label,depts,opexKeys){
  return {
    ...
    wages:emptyWages(depts), opex:emptyOpex(opexKeys),
  };
}
...
function calcWeek(week,fixed,opexKeys,depts){
  ...
  const totalWages=allWageKeys(wDepts).reduce((s,k)=>s+n(week.wages?.[k]||0),0);
...
async function loadAll(){
  ...
  try{const loc=localStorage.getItem("pl_v6");if(loc)return JSON.parse(loc);}catch(e){}
...
function generateExport(weeks,fixed,extras,mLabel,opexKeys,depts,staff,labels){
  ...
  if(staff&&staff.length>0){
    o+="--- STAFF ROSTER ---\n";
    staff.forEach(s=>{
      const weekCost=n(s.hourlyRate)*n(s.hoursPerWeek);
      o+="  "+s.name+" | "+s.type+" | "+s.dept+" | $"+n(s.hourlyRate).toFixed(2)+"/hr | "+n(s.hoursPerWeek)+"hrs/wk | Weekly cost: "+fmt(weekCost)+"\n";
    });
    const totalStaffCost=staff.reduce((sum,s)=>sum+n(s.hourlyRate)*n(s.hoursPerWeek),0);
    o+="  Total budgeted weekly wages: "+fmt(totalStaffCost)+"\n";
    o+="  Actual recorded wages: "+fmt(mc.totalWages)+"\n";
    o+="  Variance: "+fmt(mc.totalWages-totalStaffCost)+"\n\n";
  }

  o+="=== END DATA ===\n\n";
  o+="You are the COO's senior financial advisor for this e-commerce business. Produce a comprehensive written P&L analysis report. Write in full paragraphs - NOT dot points. Each section must go beyond the numbers and explain the why, the structural risk, the opportunity, and the exact action required.\n\n";
  o+="1. PROFITABILITY VERDICT - Assess the business's financial health. Is net margin sustainable? Compare to e-commerce benchmarks (10-15% net, 40-65% gross). Is the business in growth, maintenance, or risk posture?\n\n";
  o+="2. WEEK-ON-WEEK TRENDS - Analyse each week's trajectory. Identify patterns and outliers.\n\n";
  o+="3. MONEY BLEED - For every cost category, exact dollar amount and % of net revenue. Rank by impact.\n\n";
  o+="4. REVENUE QUALITY - Discount rate, refund rate, net revenue yield per gross dollar.\n\n";
  o+="5. COGS AND GROSS MARGIN - Manufacturing efficiency. Scenario if volume doubles or halves.\n\n";
  o+="6. FREIGHT EFFICIENCY - Carrier split, net shipping subsidy, quantified recovery strategy.\n\n";
  o+="7. COLLAB ROI - Minimum revenue to justify spend at 3:1 and 5:1 ROAS.\n\n";
  o+="8. WAGES BY DEPARTMENT - Wages as % of net revenue per department. Efficiency assessment.\n\n";
  o+="9. OPEX LINE BY LINE - Assess, benchmark, flag for renegotiation. Model 20% revenue decline.\n\n";
  o+="10. TOP 5 ACTIONS - Exact dollar improvement, mechanism, timeline, trade-off.\n\n";
  o+="11. MARGIN EXPANSION - Structural changes over 90 days. Specific targets.\n\n";
  o+="12. NEXT MONTH TARGETS - Exact dollar targets. Break-even calculation.\n\n";
  o+="13. NEXT WEEK BUDGET AND STAFFING PLAN - Based on the roster data provided and this week's actual wages vs budgeted wages variance, produce a recommended staffing plan for next week. For each staff member, recommend: optimal hours, department allocation, and whether to increase or reduce hours based on current revenue levels and wage efficiency. Calculate the exact weekly wage budget that would produce a 15% net margin at the current revenue run rate. Identify which employment types (full-time, part-time, casual) are over- or under-represented given the business's current operational load. If wage costs are above target, specify exactly whose hours should be adjusted and by how much, and what the dollar saving would be. If wages are below budget, assess whether under-staffing is creating operational risk. Provide a concrete, actionable staffing schedule recommendation for next week.\n\nWrite in full paragraphs. Use exact figures. Flag anomalies. Make it worth reading.";
  return o;
}
...
function generateComparativeExport(periodsData){
  ...
  o+="\n=== END DATA ===\n\nYou are the COO's senior financial advisor. Produce a comprehensive comparative P&L analysis. Write in full paragraphs.\n\n1. PERFORMANCE COMPARISON - Which period performed better and why?\n2. TREND DIRECTION - Is the business improving or deteriorating?\n3. COST STRUCTURE CHANGES - Which costs grew or shrank as % of revenue?\n4. REVENUE QUALITY COMPARISON - How did discount rates, refund rates change?\n5. MARGIN TRAJECTORY - Is gross and net margin expanding or contracting?\n6. WHAT WORKED - What improved and should be scaled?\n7. WHAT DETERIORATED - What got worse and needs intervention?\n8. FORWARD PROJECTION - What will next period look like if nothing changes?\n9. TOP 5 ACTIONS - Prioritised by comparative evidence. Exact dollar targets.\n\nUse exact figures. Be commercially direct.";
  return o;
}
...
function useBaseInp(){
  const {S,BR,TX,ff,radius}=useTheme();
  return {width:"100%",boxSizing:"border-box",background:S,border:"1px solid "+BR,color:TX,padding:"8px 10px",fontFamily:ff,fontSize:14,outline:"none",borderRadius:radius};
}

function CI({value,onChange,placeholder="0.00",tint}){
  const {S,BR,A,MU,ff,radius}=useTheme();
  const baseInp=useBaseInp();
  return(
...
function Lbl({c,children}){
  const {MU,ff}=useTheme();
  return <div style={{color:c||MU,fontFamily:ff,fontSize:11,letterSpacing:0.8,textTransform:"uppercase",marginBottom:5}}>{children}</div>;
}

function SH({children,sub}){
  const {A,BR,MU,ff}=useTheme();
  return <div style={{fontFamily:ff,fontSize:sub?9:10,letterSpacing:sub?1.5:2.5,textTransform:"uppercase",color:sub?MU:A,borderBottom:"1px solid "+BR,paddingBottom:7,marginBottom:14,marginTop:sub?16:26}}>{children}</div>;
}
...
function Badge({label,value,color,small}){
  const {S2,BR,MU,ff,radius}=useTheme();
  const {GR,RD}=useTheme();
...
// Inline editable text - faint "edit" appears on hover
function E({value,onSave,style={},multiline=false}){
  ...
  if(editing){
    if(multiline)return <textarea value={draft} onChange={e=>setDraft(e.target.value)} onBlur={commit} autoFocus
      style={{background:"transparent",border:"none",borderBottom:"1px solid "+A,color:style.color||"inherit",fontFamily:style.fontFamily||"inherit",fontSize:style.fontSize||"inherit",outline:"none",width:"100%",resize:"none",lineHeight:1.5,...style}}/>;
    return <input value={draft} onChange={e=>setDraft(e.target.value)} onBlur={commit}
      onKeyDown={e=>{if(e.key==="Enter")commit();if(e.key==="Escape"){setEditing(false);setDraft(value);}}}
      autoFocus style={{background:"transparent",border:"none",borderBottom:"1px solid "+A,color:style.color||"inherit",fontFamily:style.fontFamily||"inherit",fontSize:style.fontSize||"inherit",outline:"none",width:"100%",...style}}/>;
  }
  return(
    <span onMouseEnter={()=>setHover(true)} onMouseLeave={()=>setHover(false)}
      ...
      <span onClick={()=>setEditing(true)}
        style={{opacity:hover?0.35:0,fontSize:9,color:A,transition:"opacity 0.15s",userSelect:"none",letterSpacing:0.5,textTransform:"lowercase",fontWeight:"normal"}}>
        edit
      </span>
    </span>
  );
...
// Button component
function Btn({children,onClick,variant="outline",fullWidth=false,small=false,danger=false}){
  const {A,S2,BR,BG,RD,ff,radius}=useTheme();
  const [hover,setHover]=useState(false);
  const base={fontFamily:ff,cursor:"pointer",borderRadius:radius,letterSpacing:1.5,textTransform:"uppercase",transition:"all 0.15s",border:"none"};
  const col=danger?RD:A;
  const styles={
    outline:{...base,background:hover?col:"transparent",border:"1px solid "+col,color:hover?BG:col,padding:small?"6px 14px":"10px 20px",fontSize:small?11:12},
    filled:{...base,background:hover?col+"dd":col,border:"1px solid "+col,color:BG,padding:small?"6px 14px":"10px 20px",fontSize:small?11:12,fontWeight:"bold"},
    ghost:{...base,background:"transparent",border:"1px solid "+BR,color:hover?A:BR,padding:small?"6px 14px":"10px 20px",fontSize:small?11:12},
  };
  return <button onClick={onClick} style={{...styles[variant],width:fullWidth?"100%":"auto"}} onMouseEnter={()=>setHover(true)} onMouseLeave={()=>setHover(false)}>{children}</button>;
}
...
function WeekForm({week,onChange,fixed,opexKeys,depts,settings,onSettingsChange,labels}){
  ...
  const renameWageDept=(dKey,newLabel)=>{
    if(onSettingsChange){const nd=(settings?.wageDepts||wDepts).map(d=>d.key===dKey?{...d,label:newLabel}:d);onSettingsChange({...settings,wageDepts:nd});}
  };
  const renameWageSub=(sKey,newLabel)=>{
    if(onSettingsChange){const nd=(settings?.wageDepts||wDepts).map(d=>({...d,subs:d.subs.map(s=>s.key===sKey?{...s,label:newLabel}:s)}));onSettingsChange({...settings,wageDepts:nd});}
  };
  ...
  const opexField=(key,label)=>{
    ...
    return(
      <Fld key={key} label={<E value={label} onSave={nl=>renameOpexKey(key,nl)} style={{fontFamily:ff,fontSize:11,color:MU,textTransform:"uppercase",letterSpacing:0.8}}/>}>
  ...
  return(
    <div>
      ...
      <div style={{marginTop:14,background:S2,border:"1px solid "+BR,borderRadius:radius+1,padding:"12px 14px"}}>
        ...
        <Grid>
          <Fld label={<E value={labels.field_satchel_count} onSave={v=>labels._save("field_satchel_count",v)} style={{color:MU,fontFamily:ff,fontSize:11}}/>}>
            <input type="number" value={week.cogs.satchel_count} onChange={e=>upC("satchel_count",e.target.value)} placeholder="0" style={baseInp} onFocus={e=>e.target.style.borderColor=A} onBlur={e=>e.target.style.borderColor=BR}/>
          </Fld>
      ...
      <div style={{marginTop:10}}>
        <Fld label={<E value={labels.field_other_pkg} onSave={v=>labels._save("field_other_pkg",v)} style={{color:MU,fontFamily:ff,fontSize:11}}/>}><CI value={week.cogs.other_packaging} onChange={v=>upC("other_packaging",v)}/></Fld>
      </div>
      <Row>
      ...
      <div style={{fontFamily:ff,fontSize:11,color:MU,marginBottom:16}}>
        <E value={labels.sec_opex_sub} onSave={v=>labels._save("sec_opex_sub",v)} style={{color:MU,fontFamily:ff,fontSize:11}} multiline/>
      </div>

      <SH sub><E value={labels.sec_freight} onSave={v=>labels._save("sec_freight",v)} style={{color:MU,fontFamily:ff,fontSize:9}}/></SH>
      <div style={{fontFamily:ff,fontSize:11,color:MU,marginBottom:10}}>
        <E value={labels.sec_freight_sub} onSave={v=>labels._save("sec_freight_sub",v)} style={{color:MU,fontFamily:ff,fontSize:11}}/>
      </div>
      ...
      <div style={{fontFamily:ff,fontSize:11,color:MU,marginBottom:10}}>
        <E value={labels.sec_collabs_sub} onSave={v=>labels._save("sec_collabs_sub",v)} style={{color:MU,fontFamily:ff,fontSize:11}}/>
      </div>
      ...
      <div style={{fontFamily:ff,fontSize:11,color:MU,marginBottom:10}}>
        <E value={labels.sec_wages_sub} onSave={v=>labels._save("sec_wages_sub",v)} style={{color:MU,fontFamily:ff,fontSize:11}}/>
      </div>
      {wDepts.map(dept=>(
        <div key={dept.key} style={{marginBottom:16}}>
          <div style={{fontFamily:ff,fontSize:11,color:A,letterSpacing:1,textTransform:"uppercase",marginBottom:8,paddingBottom:4,borderBottom:"1px solid "+BR+"44"}}>
            <E value={dept.label} onSave={nl=>renameWageDept(dept.key,nl)} style={{fontFamily:ff,fontSize:11,color:A}}/>
          </div>
          <Grid>{dept.subs.map(sub=>(
            <Fld key={sub.key} label={<E value={sub.label} onSave={nl=>renameWageSub(sub.key,nl)} style={{color:MU,fontFamily:ff,fontSize:11}}/>}>
...
function FixedCostsPage({fixed,onChange,opexKeys,settings,onSettingsChange,labels}){
  ...
  const renameKey=(key,newLabel)=>{
    if(onSettingsChange){const nk=keys.map(k=>k.key===key?{...k,label:newLabel}:k);onSettingsChange({...settings,opexKeys:nk});}
  };
  const renderGroup=(groupKeys,titleKey)=>(
    <div style={{marginBottom:20}}>
      <SH sub><E value={labels[titleKey]||groupKeys[0]?.group||"Group"} onSave={v=>labels._save(titleKey,v)} style={{color:MU,fontFamily:ff,fontSize:9}}/></SH>
...
const COLOR_PRESETS=[
  {name:"Default (Dark Purple)",theme:{accent:"#d8b9ff",bg:"#0a0a0e",surface:"#12111a",surface2:"#1a1826",border:"#2a2540",text:"#e0e0e0",muted:"#777777",red:"#ff6b6b",green:"#6bffb8",yellow:"#ffd97d"}},
  ...
  {name:"Forest Dark",theme:{accent:"#86efac",bg:"#030712",surface:"#0f1b12",surface2:"#172018",border:"#2d4a32",text:"#dcfce7",muted:"#6b7280",red:"#fca5a5",green:"#86efac",yellow:"#fde68a"}},
  {name:"Midnight Rose",theme:{accent:"#fda4af",bg:"#0c0a0b",surface:"#1a1016",surface2:"#231520",border:"#4a2030",text:"#fce7f3",muted:"#9d8090",red:"#fb7185",green:"#6ee7b7",yellow:"#fde68a"}},
...
function ColorSwatch({label,colorKey,themeEdit,setThemeEdit}){
  const {MU,ff}=useTheme();
  return(
    <div style={{marginBottom:12}}>
      <div style={{fontFamily:ff,fontSize:10,letterSpacing:1,textTransform:"uppercase",color:MU,marginBottom:4}}>{label}</div>
      <div style={{display:"flex",alignItems:"center",gap:10}}>
        <input type="color" value={themeEdit[colorKey]||"#ffffff"} onChange={e=>setThemeEdit({...themeEdit,[colorKey]:e.target.value})}
          style={{width:44,height:32,border:"none",background:"none",cursor:"pointer",padding:0}}/>
        <input type="text" value={themeEdit[colorKey]||""} onChange={e=>setThemeEdit({...themeEdit,[colorKey]:e.target.value})}
          style={{background:"transparent",border:"1px solid #333",color:themeEdit[colorKey]||"#fff",padding:"5px 8px",fontFamily:"monospace",fontSize:13,borderRadius:3,outline:"none",width:100}}/>
        <div style={{width:32,height:32,borderRadius:4,background:themeEdit[colorKey]||"#888",border:"1px solid #333"}}/>
      </div>
    </div>
  );
}
...
function SettingsPage({settings,onSettingsChange,theme,onThemeChange}){
  const {S2,BR,A,MU,TX,ff,GR,RD,radius,BG}=useTheme();
  ...
  const applyTheme=()=>{onThemeChange(themeEdit);setSaved(true);setTimeout(()=>setSaved(false),2000);};
  const resetTheme=()=>{setThemeEdit({...DEFAULT_THEME});onThemeChange({...DEFAULT_THEME});};
  const applyPreset=p=>{const nt={...themeEdit,...p.theme};setThemeEdit(nt);onThemeChange(nt);};
  ...
  const editStaff=(id,field,val)=>updateStaff(staff.map(s=>s.id===id?{...s,[field]:val}:s));

  const inputStyle={background:S2,border:"1px solid "+BR,color:TX,padding:"7px 10px",fontFamily:ff,fontSize:13,outline:"none",borderRadius:radius,width:"100%",boxSizing:"border-box"};
  const tabs=["appearance","colours","staff"];
    ...
    <div>
      ...
      {activeTab==="appearance"&&(
        <div>
          <SH>Fonts</SH>
          <Grid>
            <Fld label="Title Font">
              <select value={themeEdit.titleFont||"Times New Roman"} onChange={e=>setThemeEdit({...themeEdit,titleFont:e.target.value})} style={inputStyle}>
                {FONT_OPTIONS.map(f=><option key={f} value={f}>{f}</option>)}
              </select>
            </Fld>
            <Fld label="Body / Label Font">
              <select value={themeEdit.bodyFont||"Times New Roman"} onChange={e=>setThemeEdit({...themeEdit,bodyFont:e.target.value})} style={inputStyle}>
                {FONT_OPTIONS.map(f=><option key={f} value={f}>{f}</option>)}
              </select>
            </Fld>
          </Grid>
          ...
          <div style={{display:"flex",alignItems:"center",gap:16,marginBottom:20}}>
            ...
            <div style={{width:40,height:24,background:A,borderRadius:themeEdit.borderRadius??4,border:"1px solid "+BR}}/>
          </div>
          <SH>Colour Lightness</SH>
          <div style={{fontFamily:ff,fontSize:12,color:MU,marginBottom:10}}>Shift all colours toward black (left) or white/pastel (right).</div>
          <div style={{display:"flex",alignItems:"center",gap:16,marginBottom:24}}>
            ...
            <span style={{fontFamily:ff,fontSize:11,color:MU}}>Lighter / Pastel</span>
          ...
          <div style={{display:"flex",gap:10}}>
            <button onClick={applyTheme} style={{flex:1,padding:"11px 0",background:A,border:"none",color:"#000",fontFamily:ff,fontSize:12,cursor:"pointer",borderRadius:radius,fontWeight:"bold",letterSpacing:1}}>
              {saved?"APPLIED!":"APPLY APPEARANCE"}
            </button>
            <button onClick={resetTheme} style={{padding:"11px 20px",background:"transparent",border:"1px solid "+BR,color:MU,fontFamily:ff,fontSize:12,cursor:"pointer",borderRadius:radius}}>Reset</button>
          </div>
        </div>
      )}
      ...
      {activeTab==="colours"&&(
        <div>
          <div style={{fontFamily:ff,fontSize:12,color:MU,marginBottom:20}}>Fine-tune individual colours. Use the lightness slider on the Appearance tab for global adjustments.</div>
          <Grid cols={2}>
            {[["Accent / Highlight","accent"],["Background","bg"],["Surface","surface"],["Surface 2","surface2"],["Border","border"],["Text","text"],["Muted Text","muted"],["Red / Negative","red"],["Green / Positive","green"],["Yellow / Warning","yellow"]].map(([label,key])=>(
              <ColorSwatch key={key} label={label} colorKey={key} themeEdit={themeEdit} setThemeEdit={setThemeEdit}/>
            ))}
          </Grid>
          <div style={{display:"flex",gap:10,marginTop:20}}>
            <button onClick={applyTheme} style={{flex:1,padding:"11px 0",background:A,border:"none",color:"#000",fontFamily:ff,fontSize:12,cursor:"pointer",borderRadius:radius,fontWeight:"bold",letterSpacing:1}}>
              {saved?"APPLIED!":"APPLY COLOURS"}
            </button>
          </div>
        </div>
      )}
      ...
      {activeTab==="staff"&&(
        <div>
          <div style={{fontFamily:ff,fontSize:13,color:MU,marginBottom:20,lineHeight:1.8}}>
            Enter your staff roster. This powers the budget planning section in the weekly export, giving Claude context to recommend next-week staffing based on wage efficiency.
          </div>
          {staff.map(s=>{
            const weekCost=n(s.hourlyRate)*n(s.hoursPerWeek);
            return(
              <div key={s.id} style={{background:S2,border:"1px solid "+BR,borderRadius:radius+2,padding:"14px 16px",marginBottom:12}}>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr 1fr auto",gap:10,alignItems:"end"}}>
                  <Fld label="Name">
                    <input value={s.name} onChange={e=>editStaff(s.id,"name",e.target.value)} style={inputStyle}/>
                  </Fld>
                  <Fld label="Type">
                    <select value={s.type} onChange={e=>editStaff(s.id,"type",e.target.value)} style={inputStyle}>
                      <option value="fulltime">Full Time</option>
                      <option value="parttime">Part Time</option>
                      <option value="casual">Casual</option>
                    </select>
                  </Fld>
                  <Fld label="Hourly Rate ($)">
                    <input type="number" value={s.hourlyRate} onChange={e=>editStaff(s.id,"hourlyRate",e.target.value)} style={inputStyle}/>
                  </Fld>
                  <Fld label="Hours / Week">
                    <input type="number" value={s.hoursPerWeek} onChange={e=>editStaff(s.id,"hoursPerWeek",e.target.value)} style={inputStyle}/>
                  </Fld>
                  <div>
                    <div style={{fontFamily:ff,fontSize:10,color:MU,letterSpacing:1,textTransform:"uppercase",marginBottom:4}}>Weekly Cost</div>
                    <div style={{fontFamily:ff,fontSize:15,color:GR,paddingTop:5}}>{fmtD(weekCost)}</div>
                  </div>
                  <button onClick={()=>removeStaff(s.id)} style={{background:"transparent",border:"1px solid "+RD,color:RD,padding:"6px 10px",fontFamily:ff,fontSize:11,cursor:"pointer",borderRadius:radius,alignSelf:"end"}}>Remove</button>
                </div>
              </div>
            );
          })}
          <button onClick={addStaff} style={{width:"100%",padding:"11px 0",background:"transparent",border:"1px solid "+A,color:A,fontFamily:ff,fontSize:12,cursor:"pointer",borderRadius:radius,letterSpacing:1.5,marginTop:4}}>
            + ADD STAFF MEMBER
          </button>
          <div style={{marginTop:16,padding:"12px 16px",background:S2,border:"1px solid "+BR,borderRadius:radius+1}}>
  ...
  return(
    <div>
      <div style={{display:"flex",gap:8,marginBottom:20}}>
        {[labels.overview_part1,labels.overview_part2].map((lbl,i)=>(
          <button key={i} onClick={()=>setPart2(i===1)}
            ...
            <E value={lbl} onSave={v=>labels._save(i===0?"overview_part1":"overview_part2",v)} style={{fontFamily:ff,fontSize:11,color:part2===(i===1)?"#000":MU}}/>
          </button>
        ))}
      </div>
      {!part2&&(
        <div>
          ...
          <Row><Badge label="Net Revenue" value={mc.netRevenue} color={A}/><Badge label="Gross Profit" value={mc.grossProfit}/><Badge label="Total Expenses" value={-mc.totalExpenses} color={RD}/><Badge label="Net Profit" value={mc.netProfit}/></Row>
          ...
          <div style={{overflowX:"auto"}}>
            <table style={{width:"100%",borderCollapse:"collapse",fontFamily:ff,fontSize:13}}>
              <thead><tr style={{borderBottom:"1px solid "+BR}}>{["Week","Date Range","Revenue","COGS","Gross","GP%","Freight","Wages","OPEX","Net Profit","NP%"].map(h=><th key={h} style={{padding:"8px 10px",color:MU,fontWeight:"normal",fontSize:10,letterSpacing:1,textTransform:"uppercase",textAlign:"right",whiteSpace:"nowrap"}}>{h}</th>)}</tr></thead>
              <tbody>
          ...
          {[["COGS",mc.totalCOGS,"#ff9ecd"],["Freight",mc.totalFreight,RD],["Collabs",mc.totalCollabs,"#ffd97d"],["Wages",mc.totalWages,"#e0a0ff"],["OPEX",mc.totalOPEX,A]].map(([lbl,val,col])=>(
          ...
          <div style={{display:"flex",gap:10,marginTop:24}}>
            <button onClick={onExport} style={{flex:1,padding:"13px 0",background:"transparent",border:"1px solid "+A,color:A,fontFamily:ff,fontSize:12,cursor:"pointer",borderRadius:radius,letterSpacing:1.5,textTransform:"uppercase"}}>
            ...
            <button onClick={copySummary} style={{flex:1,padding:"13px 0",background:S2,border:"1px solid "+BR,color:TX,fontFamily:ff,fontSize:12,cursor:"pointer",borderRadius:radius,letterSpacing:1.5,textTransform:"uppercase"}}>
      ...
      {part2&&(
        <div>
          ...
          <Grid>{keys.map(({key,label})=><Fld key={key} label={label}><CI value={extras?.opex?.[key]||""} onChange={v=>onExtrasChange({...extras,opex:{...extras?.opex,[key]:v}})}/></Fld>)}</Grid>
          ...
          <div style={{marginTop:20,padding:"14px 16px",background:S2,border:"1px solid "+BR,borderRadius:radius+1}}>
            <Row><Badge small label="Net Revenue" value={mc.netRevenue} color={A}/><Badge small label="Total Expenses" value={-mc.totalExpenses} color={RD}/><Badge small label="Net Profit" value={mc.netProfit}/><Pct small label="Net Margin" value={mc.netMargin}/></Row>
          </div>
        </div>
      )}
    </div>
...
function VisualisePage({weeks,fixed,allMonthData,opexKeys,depts}){
  ...
  const baseMetrics=[
    ...
    {id:"discounts",label:"Discounts"},{id:"refunds",label:"Refunds"},{id:"shipInc",label:"Shipping Income"},
  ];
  ...
  if(view==="weekly"){
    ...
    points=sk.map(k=>{
      const md=allMonthData[k];const wks=md.weeks||[];const mc=calcMonth(wks,fixed,md.extras,keys,wDepts);
  ...
  return(
    <div>
      <div style={{display:"flex",gap:12,flexWrap:"wrap",marginBottom:20,alignItems:"flex-end"}}>
        <div><Lbl>Metric</Lbl>
          <select value={metric} onChange={e=>setMetric(e.target.value)} style={{background:S2,border:"1px solid "+BR,color:TX,padding:"9px 12px",fontFamily:ff,fontSize:13,outline:"none",borderRadius:4,maxWidth:280}}>
            <optgroup label="Profit & Revenue">{baseMetrics.slice(0,6).map(m=><option key={m.id} value={m.id}>{m.label}</option>)}</optgroup>
            <optgroup label="Revenue Detail">{baseMetrics.slice(6).map(m=><option key={m.id} value={m.id}>{m.label}</option>)}</optgroup>
...
function ComparePage({allMonthData,fixed,opexKeys,depts,labels}){
  ...
  const [mode,setMode]=useState("months"); // months | weeks | custom
  ...
  if(mode==="months"){periodA=getMC(mA);periodB=getMC(mB);}
    ...
    if(md?.weeks){
      const wks=md.weeks;
      const wkCalc=(idx)=>{if(!wks[idx])return null;const c=calcWeek(wks[idx],fixed,keys,wDepts);return{label:wks[idx].label+" ("+wks[idx].dateRange+")",mc:{...c,weekCalcs:[c],extraOpex:0}};};
  ...
  const fmt=v=>"$"+Math.abs(v).toLocaleString("en-AU",{minimumFractionDigits:2,maximumFractionDigits:2});
  const metrics=[
    {label:"Net Revenue",key:"netRevenue",hb:true},{label:"Gross Sales",key:"gross",hb:true},{label:"Discounts",key:"discounts",hb:false},
    {label:"Gross Profit",key:"grossProfit",hb:true},{label:"Gross Margin %",key:"grossMargin",hb:true,isPct:true},{label:"Total COGS",key:"totalCOGS",hb:false},
    {label:"Total Freight",key:"totalFreight",hb:false},{label:"Total Collabs",key:"totalCollabs",hb:false},{label:"Total Wages",key:"totalWages",hb:false},
    {label:"Total OPEX",key:"totalOPEX",hb:false},{label:"Total Expenses",key:"totalExpenses",hb:false},{label:"Net Profit",key:"netProfit",hb:true},{label:"Net Margin %",key:"netMargin",hb:true,isPct:true},
  ];
  const diffColor=(a,b,hb)=>{if(a===b)return MU;return (a>b)===hb?GR:RD;};

  const handleExport=()=>{
    if(!periodA||!periodB)return;
    navigator.clipboard.writeText(generateComparativeExport([periodA,periodB]));
    setCopied(true);setTimeout(()=>setCopied(false),3000);
  };

  const inputStyle={background:S2,border:"1px solid "+BR,color:TX,padding:"7px 10px",fontFamily:ff,fontSize:13,outline:"none",borderRadius:radius};

  if(!allKeys.length)return <div style={{textAlign:"center",padding:"60px 20px",color:MU,fontFamily:ff,fontSize:13}}>Enter data for at least one month to use comparative analysis.</div>;
    ...
    <div>
      ...
      {mode==="months"&&(
        <div style={{display:"flex",gap:12,flexWrap:"wrap",marginBottom:24,alignItems:"flex-end"}}>
          <div><Lbl>Period A</Lbl><select value={mA} onChange={e=>setMA(e.target.value)} style={{...inputStyle,borderColor:A}}>{allKeys.map(k=><option key={k} value={k}>{allMonthData[k]?.label||k}</option>)}</select></div>
          <div style={{fontFamily:ff,fontSize:18,color:MU,paddingBottom:8}}>vs</div>
          <div><Lbl>Period B</Lbl><select value={mB} onChange={e=>setMB(e.target.value)} style={inputStyle}>{allKeys.map(k=><option key={k} value={k}>{allMonthData[k]?.label||k}</option>)}</select></div>
        </div>
      )}
      ...
      {mode==="weeks"&&(
        <div style={{display:"flex",gap:12,flexWrap:"wrap",marginBottom:24,alignItems:"flex-end"}}>
          <div><Lbl>Month</Lbl>
            <select value={wMonthIdx} onChange={e=>setWMonthIdx(parseInt(e.target.value))} style={inputStyle}>
              {allKeys.map((k,i)=><option key={k} value={i}>{allMonthData[k]?.label||k}</option>)}
            </select>
          </div>
          <div><Lbl>Week A</Lbl>
            <select value={wA} onChange={e=>setWA(parseInt(e.target.value))} style={{...inputStyle,borderColor:A}}>
              {(allMonthData[allKeys[wMonthIdx]]?.weeks||[]).map((w,i)=><option key={i} value={i}>{w.label} ({w.dateRange})</option>)}
            </select>
          </div>
          <div style={{fontFamily:ff,fontSize:18,color:MU,paddingBottom:8}}>vs</div>
          <div><Lbl>Week B</Lbl>
            <select value={wB} onChange={e=>setWB(parseInt(e.target.value))} style={inputStyle}>
              {(allMonthData[allKeys[wMonthIdx]]?.weeks||[]).map((w,i)=><option key={i} value={i}>{w.label} ({w.dateRange})</option>)}
            </select>
          </div>
        </div>
      ...
      {mode==="custom"&&(
        <div style={{marginBottom:24}}>
          <div style={{fontFamily:ff,fontSize:12,color:MU,marginBottom:16}}>Select any date ranges. Data from weeks that overlap each range will be summed for comparison.</div>
          <div style={{display:"flex",gap:20,flexWrap:"wrap"}}>
            <div style={{background:S2,border:"1px solid "+A,borderRadius:radius+2,padding:"14px 16px",flex:1,minWidth:220}}>
              ...
              <Grid><Fld label="From"><input type="date" value={dateA1} onChange={e=>setDateA1(e.target.value)} style={inputStyle}/></Fld><Fld label="To"><input type="date" value={dateA2} onChange={e=>setDateA2(e.target.value)} style={inputStyle}/></Fld></Grid>
            </div>
            <div style={{background:S2,border:"1px solid "+BR,borderRadius:radius+2,padding:"14px 16px",flex:1,minWidth:220}}>
              ...
              <Grid><Fld label="From"><input type="date" value={dateB1} onChange={e=>setDateB1(e.target.value)} style={inputStyle}/></Fld><Fld label="To"><input type="date" value={dateB2} onChange={e=>setDateB2(e.target.value)} style={inputStyle}/></Fld></Grid>
            </div>
          </div>
          <div style={{fontFamily:ff,fontSize:11,color:MU,marginTop:12}}>Custom range comparison sums all weeks whose date ranges fall within your selected period. Ensure data is entered for those months first.</div>
        </div>
      )}
            ...
            <table style={{width:"100%",borderCollapse:"collapse",fontFamily:ff,fontSize:13}}>
              ...
              <tbody>
                {metrics.map(({label,key,hb,isPct})=>{
                  ...
                  const col=diffColor(aVal,bVal,hb);
                  return(
...
function ReportsPage({monthData,fixed,onSave,onExport,opexKeys,depts}){
  ...
  if(!allKeys.length)return <div style={{textAlign:"center",padding:"60px 20px",color:MU,fontFamily:ff}}><div style={{fontSize:28,marginBottom:12,opacity:0.3}}>-</div><div>No saved data yet.</div><div style={{fontSize:13,marginTop:8}}>Data saves automatically as you enter it.</div></div>;
      ...
      {allKeys.map(key=>{
        ...
        return(
          <div key={key} style={{border:"1px solid "+BR,borderRadius:radius+2,marginBottom:10,overflow:"visible"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"14px 16px",background:isOpen?S2:"transparent",borderRadius:isOpen?(radius+2)+" "+(radius+2)+" 0 0":(radius+2)}}>
            ...
            {isOpen&&(
              <div style={{padding:"20px 18px",borderTop:"1px solid "+BR,background:S}}>
                {isEdit?(
                  <div>
                    ...
                    {editWeeks[activeEditWeek]&&(
                      <div style={{background:S2,border:"1px solid "+BR,borderRadius:radius+2,padding:"20px"}}>
                        <WeekForm week={editWeeks[activeEditWeek]} onChange={updated=>{const arr=[...editWeeks];arr[activeEditWeek]=updated;setEditWeeks(arr);}} fixed={fixed} opexKeys={keys} depts={wDepts} labels={{...DEFAULT_LABELS,_save:()=>{}}}/>
                      </div>
                    )}
                  ...
                  <div>
                    <Row><Badge small label="Revenue" value={mc.netRevenue} color={A}/><Badge small label="Gross Profit" value={mc.grossProfit}/><Badge small label="Total Expenses" value={-mc.totalExpenses} color={RD}/><Badge small label="Net Profit" value={mc.netProfit}/></Row>
...
export default function App(){
  ...
  // Labels with save callback
  const labels={...labelsRaw,_save:(key,val)=>{
    const nl={...labelsRaw,[key]:val};setLabelsRaw(nl);
    autoSave(monthData,fixed,{...settings,labels:nl});
  }};
  ...
  const updateWeeks=nw=>{const updated={...monthData,[curKey]:{...curEntry,weeks:nw,label:selMonth.label,lastSaved:new Date().toLocaleString("en-AU"),extras:curExtras}};setMonthData(updated);autoSave(updated,fixed,settings);};
  const updateExtras=ne=>{const updated={...monthData,[curKey]:{...curEntry,weeks:curWeeks,extras:ne,label:selMonth.label,lastSaved:new Date().toLocaleString("en-AU")}};setMonthData(updated);autoSave(updated,fixed,settings);};
  ...
  const updateTheme=nt=>{setThemeRaw(nt);const ns={...settings,theme:nt};setSettings(ns);autoSave(monthData,fixed,ns);};
  ...
  if(!authed)return(
    <ThemeContext.Provider value={theme}>
      <PasswordScreen onAuth={()=>setAuthed(true)}/>
    </ThemeContext.Provider>
  );

  const {A,BG,S,S2,BR,TX,MU,ff,ffTitle,radius}=theme;

  const TABS=[
    {id:"input",label:labels.tab_input},{id:"overview",label:labels.tab_overview},{id:"visualise",label:labels.tab_visualise},
    {id:"compare",label:labels.tab_compare},{id:"fixed",label:labels.tab_fixed},
    {id:"reports",label:labels.tab_reports+" ("+Object.keys(monthData).length+")"},
    {id:"settings",label:labels.tab_settings},
  ];
        ...
        <div style={{minHeight:"100vh",background:BG,color:TX,fontFamily:ff}}>
            ...
            <div style={{maxWidth:1200,margin:"0 auto",padding:"22px 0 0"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-end",flexWrap:"wrap",gap:12}}>
                <div>
                  ...
                  <h1 style={{margin:0,fontSize:24,fontWeight:"normal",letterSpacing:2,color:TX,textTransform:"uppercase",fontFamily:ffTitle}}>
                    <E value={labels.header_title} onSave={v=>labels._save("header_title",v)} style={{color:TX,fontFamily:ffTitle,fontSize:24}}/>
                  </h1>
                </div>
                <div style={{display:"flex",gap:10,alignItems:"center",flexWrap:"wrap"}}>
                  ...
                  <div style={{background:S2,border:"1px solid "+BR,borderRadius:radius,padding:"7px 12px",fontSize:13,color:mc.netProfit>=0?theme.GR:theme.RD,fontFamily:ff}}>
                    MTD: {fmtD(mc.netProfit)}
                  </div>
                  {saveMsg&&<div style={{fontFamily:ff,fontSize:11,color:theme.GR,letterSpacing:1}}>{saveMsg}</div>}
                </div>
              </div>
              <div style={{display:"flex",gap:0,marginTop:18,flexWrap:"wrap"}}>
                {TABS.map(t=>(
                  <button key={t.id} onClick={()=>setTab(t.id)}
                    style={{padding:"10px 16px",background:"transparent",border:"none",borderBottom:tab===t.id?"2px solid "+A:"2px solid transparent",color:tab===t.id?A:MU,fontFamily:ff,fontSize:11,cursor:"pointer",letterSpacing:1.5,marginBottom:-1,textTransform:"uppercase",whiteSpace:"nowrap"}}>
                    {t.label}
                  </button>
                ))}
              </div>
          ...
          <div style={{maxWidth:1200,margin:"0 auto",padding:"28px 32px"}}>
              ...
              <div>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:12,marginBottom:20}}>
                  <div style={{fontFamily:ff,fontSize:11,color:MU,letterSpacing:1}}>
                    <E value={selMonth?.label+" - "+(labels.header_subtitle||"weeks auto-dated Mon-Sun")} onSave={v=>labels._save("header_subtitle",v.split(" - ").slice(1).join(" - ")||v)} style={{fontFamily:ff,fontSize:11,color:MU}}/>
                  </div>
                  <div style={{display:"flex",gap:8}}>
                    <button onClick={()=>handleExport()}
                      style={{padding:"9px 16px",background:"transparent",border:"1px solid "+A,color:copied?"#000":A,background:copied?A:"transparent",fontFamily:ff,fontSize:11,cursor:"pointer",borderRadius:radius,letterSpacing:1.5,textTransform:"uppercase"}}>
                      <E value={labels.btn_generate_export} onSave={v=>labels._save("btn_generate_export",v)} style={{fontFamily:ff,fontSize:11,color:copied?"#000":A}}/>{copied?" - Copied!":""}
                    </button>
                  </div>
                </div>
                <div style={{display:"flex",gap:8,marginBottom:20,flexWrap:"wrap"}}>
                  {curWeeks.map((w,i)=>{
                    ...
                    return(
                      <button key={i} onClick={()=>setActiveWeek(i)}
                        ...
                        <div style={{fontSize:11,color:c.netProfit!==0?(c.netProfit>=0?theme.GR:theme.RD):MU,marginTop:2}}>{c.netProfit!==0?fmtS(c.netProfit):"No data"}</div>
                      </button>
                    );
                  })}
                </div>
                {curWeeks[activeWeek]&&(
                  <div style={{background:S,border:"1px solid "+BR,borderRadius:radius+4,padding:"24px 28px"}}>
                    <div style={{fontFamily:ff,fontSize:10,letterSpacing:2,color:A,textTransform:"uppercase",marginBottom:16}}>{curWeeks[activeWeek].label} - {curWeeks[activeWeek].dateRange}</div>
                    <WeekForm week={curWeeks[activeWeek]} onChange={updated=>{const nw=[...curWeeks];nw[activeWeek]=updated;updateWeeks(nw);}} fixed={fixed} opexKeys={opexKeys} depts={wageDepts} settings={settings} onSettingsChange={updateSettings} labels={labels}/>
                  </div>
                )}
              </div>
            )}
            ...
            {tab==="settings"&&!loading&&(
              <div style={{background:S,border:"1px solid "+BR,borderRadius:radius+4,padding:"24px 28px"}}>
                <SettingsPage settings={settings} onSettingsChange={st=>{updateSettings({...settings,...st});}} theme={themeRaw} onThemeChange={updateTheme}/>
              </div>
            )}
          </div>
        </div>
