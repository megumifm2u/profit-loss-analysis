...
function SH({children,sub,divider=true}){
  const {A,BR,MU,ff,TX}=useTheme();
  if(sub) return(
    <div style={{fontFamily:ff,fontSize:9,letterSpacing:1.5,textTransform:"uppercase",color:MU,marginBottom:10,marginTop:18,paddingLeft:14,borderLeft:"2px solid "+MU+"44"}}>
      {children}
    </div>
  );
  return(
    <div style={{marginTop:28,marginBottom:0}}>
      ...
      <div style={{display:"flex",alignItems:"center",gap:10,padding:"10px 0 10px",borderBottom:"2px solid "+A+"66"}}>
        ...
        <div style={{fontFamily:ff,fontSize:10,letterSpacing:2.5,textTransform:"uppercase",color:A,fontWeight:"bold"}}>
          {children}
        </div>
      </div>
    </div>
...
function DiscountBreakdown({week,onChange,labels}){
  ...
  const allocatedDisc=dr.serviceRecoveryRetail+dr.marketingDisc+dr.staffDisc+dr.promoDisc;
    ...
    <Accordion title={<E value={labels.disc_section} onSave={v=>labels._save("disc_section",v)} style={{fontFamily:ff,fontSize:10,color:A}}/>} accent>
      ...
      <div style={{background:S2,borderRadius:radius+1,padding:"12px 14px",marginBottom:16}}>
        ...
        <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
          {[["Service Rec.",dr.serviceRecoveryRetail,RD],["Marketing",dr.marketingDisc,YL],["Staff",dr.staffDisc,A],["Promo",dr.promoDisc,GR]].map(([l,v,c])=>(
        ...
        {Math.abs(unallocated)>0.01&&(
          <div style={{marginTop:8,fontFamily:ff,fontSize:11,color:Math.abs(unallocated)>1?RD:MU}}>
            Unallocated: {fmtD(unallocated)} {Math.abs(unallocated)>1?"- allocate remaining amount across buckets below":"- rounding ok"}
          </div>
        )}
      </div>
      ...
      {buckets.map(bucket=>{
        ...
        const reclassBadge={service_recovery:"Reclassified as: COGS / Operational Expense",marketing:"Reclassified as: Marketing Expense (OPEX)",staff:"Reclassified as: Staff Benefits (Wages)",promotional:"Stays as: Revenue Deduction"}[bucket.id];
        return(
...
function WeekForm({week,onChange,fixed,opexKeys,depts,settings,onSettingsChange,labels}){
  ...
  return(
    <div>
      ...
      <div style={{fontFamily:ff,fontSize:11,color:MU,marginBottom:8,paddingLeft:16}}><E value={labels.sec_opex_sub} onSave={v=>labels._save("sec_opex_sub",v)} style={{color:MU,fontFamily:ff,fontSize:11}} multiline/></div>
...
function TargetsPanel({calc,week,labels}){
  ...
  const metrics=[
    ...
    {label:"COGS % of Revenue",actual:cogsPct,target:targets.cogs_pct_target,unit:"%",higherBetter:false},
    {label:"OPEX % of Revenue",actual:opexPct,target:targets.opex_pct_target,unit:"%",higherBetter:false},
    {label:"Wages % of Revenue",actual:wagesPct,target:targets.wages_pct_target,unit:"%",higherBetter:false},
    {label:"Promo Discount Rate",actual:promoRate,target:targets.promo_disc_rate_max,unit:"%",higherBetter:false},
    {label:"Refund Rate",actual:refundRate,target:targets.refund_rate_max,unit:"%",higherBetter:false},
  ].filter(m=>m.actual>0||m.target>0);

  const alerts=[];
  const srOrders=c.discReclass?.serviceRecoveryOrders||0;
  const srCostPer=srOrders>0?(c.discReclass?.serviceRecoveryCOGS||0)/srOrders:0;
  if(srOrders>=targets.service_recovery_max_orders) alerts.push({msg:`Service Recovery: ${srOrders} orders this week (threshold: ${targets.service_recovery_max_orders})`,sev:"warn"});
  if(srCostPer>=targets.service_recovery_cost_alert) alerts.push({msg:`Avg service recovery cost ${fmtD(srCostPer)}/order (threshold: ${fmtD(targets.service_recovery_cost_alert)})`,sev:"warn"});
  if(promoRate>targets.promo_disc_rate_max) alerts.push({msg:`Promo discount rate ${promoRate.toFixed(1)}% exceeds ${targets.promo_disc_rate_max}% target — eroding margin`,sev:"alert"});
  if(refundRate>targets.refund_rate_max) alerts.push({msg:`Refund rate ${refundRate.toFixed(1)}% exceeds ${targets.refund_rate_max}% target`,sev:"warn"});
  if(c.netMargin<targets.net_margin_target&&c.netRevenue>0) alerts.push({msg:`Net margin ${c.netMargin.toFixed(1)}% below ${targets.net_margin_target}% target — need ${fmtD((targets.net_margin_target/100-c.netMargin/100)*c.netRevenue)} more profit`,sev:"alert"});
  ...
  return(
    <div style={{marginTop:16}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(160px,1fr))",gap:8,marginBottom:alerts.length?14:0}}>
        {metrics.map(m=>{
          const ok=m.higherBetter?m.actual>=m.target:m.actual<=m.target;
          const col=ok?GR:(Math.abs(m.actual-m.target)<m.target*0.1?YL:RD);
          const barPct=Math.min(100,m.target>0?(m.actual/m.target)*100:0);
          return(
            <div key={m.label} style={{background:BG,border:"1px solid "+BR,borderRadius:radius+1,padding:"8px 10px"}}>
              ...
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"baseline",marginBottom:5}}>
                ...
                <span style={{fontFamily:ff,fontSize:9,color:MU}}>target {m.target}{m.unit}</span>
              </div>
      ...
      {alerts.length>0&&(
        <div style={{display:"flex",flexDirection:"column",gap:6,marginTop:8}}>
          {alerts.map((a,i)=>(
            <div key={i} style={{background:a.sev==="alert"?RD+"15":YL+"15",border:"1px solid "+(a.sev==="alert"?RD:YL)+"44",borderRadius:radius,padding:"8px 12px",display:"flex",alignItems:"flex-start",gap:8}}>
              <span style={{color:a.sev==="alert"?RD:YL,fontSize:12,lineHeight:1,marginTop:1}}>{a.sev==="alert"?"⚠":"!"}</span>
              <span style={{fontFamily:ff,fontSize:11,color:a.sev==="alert"?RD:YL,lineHeight:1.5}}>{a.msg}</span>
            </div>
          ))}
        </div>
      )}
...
function DiscountBreakdown({week,onChange,labels}){
  ...
  const applyDiscShopify=()=>{
    ...
    let filled=0;
    lines.forEach(line=>{
      // try: CODE\t$amount\torders or CODE,amount,orders
      const parts=line.split(/[\t,]/).map(p=>p.trim().replace(/[$,]/g,""));
      if(parts.length<2)return;
      const code=parts[0].toUpperCase();
      const amount=parseFloat(parts[1])||0;
      const orders=parseInt(parts[2])||0;
      const reg=DISCOUNT_CODE_REGISTRY.find(c=>c.id===code||code.includes(c.id)||c.id.includes(code));
      if(reg&&amount>0){
        upCode(reg.id,"retailValue",String(amount));
        if(orders>0)upCode(reg.id,"orders",String(orders));
        filled++;
      }
    });
    if(filled===0){setDiscMsg("No matching codes found — check format: CODE  $amount  orders");return;}
    setDiscMsg(`Auto-filled ${filled} code${filled>1?"s":""}`);setTimeout(()=>setDiscMsg(""),3000);
  };
