...
// POST /api/shopify
// Body: { clientId, clientSecret, startDate, endDate }
...
export default async function handler(req, res) {
  ...
  // ── Get access token ──────────────────────────────────────────────────────
  // If a direct access token is provided use it, otherwise exchange client credentials
  ...
  try {
    while (nextUrl) {
      ...
      if (r.status === 401) {
        return res.status(401).json({ error: "Invalid credentials — token was rejected by Shopify." });
      }
      if (!r.ok) {
